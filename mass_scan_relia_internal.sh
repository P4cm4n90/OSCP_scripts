...
while read i ; do
	...
	proxychains -q -f /etc/proxychains.conf nmap -T5 -Pn $ports "$i" -oN "$scan_dir/$i/scans/nmap_full$ports_out" &
...
while read i ; do
	...
	proxychains -q -f /etc/proxychains.conf nmap -sCV -p $ports $i -oN "$scan_dir/$i/scans/nmap_complete$ports_out" &
done < $ip_list
